...
describe('Setup Action', () => {
    ...
    it('should download and extract the tool', async () => {
        ...
        mockDownloadTool.mockResolvedValueOnce('/tmp/omashu-1.0.0-linux_amd64.tar.gz')

        await run();

        const expectedUrl = 'https://github.com/meigma/omashu/releases/download/v1.0.0/omashu-1.0.0-linux_amd64.tar.gz'
        expect(mockDownloadTool).toHaveBeenCalledWith(expectedUrl)
        expect(mockExtractTar).toHaveBeenCalledWith('/tmp/omashu-1.0.0-linux_amd64.tar.gz', '/usr/local/bin')
    })
